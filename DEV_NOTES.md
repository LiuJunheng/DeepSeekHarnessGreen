...
- **版本**：`GREEN_VERSION = 1.0.20`（launcher.py 内，唯一版本来源；`GREEN_VERSION_DATE` 打包当天自动回写；`build_release_zip.py` 已改为**从 launcher.py 读取版本号**，不再硬编码脚本内版本，避免 zip 名与真实版本不一致）。GitHub + Gitee 双平台已发到 **v1.0.20**（2026-08-27：GitHub Release id=377929283 / Gitee Release id=918189，zip 17,742,585 B）。上一版 v1.0.19（GitHub Release id=377753678 / Gitee Release id=916774，zip 17,745,739 B）。
- **官方 dsh 版本检测动态化（2026-08-29）**：官方发布**不再只走 npm 的 latest/next 标签**——本次 `0.1.2-alpha.1` 只发在 GitHub Releases（tag `dsh-v0.1.2-alpha.1`），npm 仍停在 `0.1.1-rc.2`，旧检测逻辑（只看 npm dist-tags）永远看不到 → 「检查更新」改为**动态检测所有标签**：`dsh_github_releases()` 分页拉官方 GitHub Releases 全部 tag（per_page=100×5 页；`_dsh_tag_to_version()` 兼容 `dsh-v`/`v`/裸版本号三种前缀）+ `dsh_npm_versions()` 拉 npm 全部已发布版本（用于判断某 tag 是否可安装）→ 合并成动态候选列表（可安装优先、版本从新到旧）→ GUI 改为**可滚动 Treeview 列表**列出（版本/标签来源/发布时间/可安装），选中未发布到 npm 的源码 tag 时给明确提示并支持「打开 GitHub 发布页」；已发布到 npm 的才允许进「确认升级」。**注意**：GitHub 源码 tag 对应版本**不一定在 npm**（本次 0.1.2-alpha.1 即如此，npm `versions` 列表里没有它，`npm install @…@0.1.2-alpha.1` 会 ETARGET），只能等官方同步发布后才能在启动器里自动安装。
- **README 面向使用者精简重写（2026-08-29）**：README 停更导致残留旧版信息（如"Gitee tag v1.0.14"）。按使用者视角重写：**下载入口只给 GitHub/Gitee 双平台地址、不再强调版本号**；砍掉目录树/配置表/按钮全表/插件详解等开发侧细节（细节在 DEV_NOTES），聚焦「绿色版优势与特点」（双击即用/绿色便携/8 款内置插件/双通道更新/局域网访问），内置插件表补上 `dsh-ollama`（8 款）；README_EN 仅同步去掉版本强调的两处。约定：README.md 面向使用者、突出亮点、不给版本号；发布相关细节放 DEV_NOTES.md。**同日追加**：标题改为「DeepSeek Harness **桌面**绿色整合版启动器」；亮点速览新增「桌面版界面体验」——默认以独立桌面窗口打开（desktop-shell.py 内嵌 WebView2，完全脱离浏览器，标题「DeepSeek Harness 桌面版」，`open_method` 默认 `desktop`），可一键切「网页窗口」用系统浏览器打开原始 WebUI；快速上手与按钮表（「桌面窗口 / 网页窗口」）同步更新。
...
- **仓库**：GitHub `LiuJunheng/DeepSeekHarnessGreen` + Gitee `liujunheng/DeepSeekHarnessGreen`。**发布源平台 = Gitee**（2026-08-27，用户反向设置：Gitee 自动推送 GitHub、GitHub 自动同步 Gitee 的仓库与 tag）——以后代码/tag 只需提交推送 Gitee，GitHub 侧自动同步；**但 Release 仍需双平台分别上传**（GitHub 自动同步只同步代码/tag，不会复制 Release 资产）。协议统一 Apache-2.0（绿色版外壳 + 全部内置插件）。
- **发布凭据**：GitHub Release 用 `GH_TOKEN` 环境变量（可自动建 Release/传资产）；Gitee 需用户提供 PAT（保存在 project memory，勿写死进代码/文档）。GitHub 上传需确认 gh CLI 或 `GH_TOKEN` 就位后才执行上传动作。
...
43. **官方 dsh 版本发布"npm 与 GitHub 不同步"，只查 npm dist-tags 会漏更新**（2026-08-29 实测）：官方把每个版本都发到 GitHub Releases（tag 形如 `dsh-v<version>`），但**不一定**同步发布到 npm——本次 `0.1.2-alpha.1`（会话流/图片上传等大量改进）：GitHub 2026-08-27 已发、npm `versions` 至今仍只有 `0.1.1-rc.2`，只看 `latest`/`next` 永远检测不到。且**源码 tag 无法直接安装**：npm 没有该版本时 `npm install @deepseek-ai/dsh@<version>` 报 ETARGET；GitHub release 的 tarball 是 **monorepo 源码包**（根 package.json 是 `@deepseek-ai/dsh-root` + workspaces，需 pnpm build），也装不成 dsh 包。**正解 = 「检查更新」动态化**（launcher.py 已实现）：`dsh_github_releases()`（GitHub API `releases?per_page=100&page=N` 分页拉全部 tag，`_dsh_tag_to_version()` 解析 `dsh-v`/`v`/裸版本号前缀）+ `dsh_npm_versions()`（`npm view … versions --json` 全量版本集合）→ 合并候选（可安装优先、版本从新到旧）→ GUI 可滚动 Treeview 列出（版本/标签来源/发布时间/可安装）+「打开 GitHub 发布页」；未发布到 npm 的版本点确认升级给明确提示。坑 26 教训再次应验：凡依赖第三方列表接口取"最新/全部"都要防顺序假设 + 分页截断。
...
41. **发布约定（v1.0.19 记录 → v1.0.20 起用户反向设置改为 Gitee 为源平台）：Gitee↔GitHub 自动同步方向会变，务必以当次记录为准**——曾设"GitHub 为源、Gitee 从 GitHub 自动同步"（v1.0.19），2026-08-27 用户反向设回"Gitee 为源、GitHub 自动同步 Gitee"：此后代码/tag 只需提交推送 **Gitee**、GitHub 侧自动同步，不再手动推 GitHub（也不再用 Git Data API 续推 GitHub，见坑 40）。**但 Release 资产不会被自动同步，双平台都要分别上传绿色 zip**（Gitee Release 用 `runtime/tmp/gitee_upload_release.py`；GitHub Release 需确认 `GH_TOKEN` / gh CLI 就位后上传）。核对上传完成：Gitee Release 附件 zip 字节数 = 本地 zip 字节数；GitHub Release 附件同样核对。
40. **Git Data API 断点续推：远端 master 会变成"本地没有的改写 SHA"，`rev-list <remote>..master` 直接报 `Invalid revision range`**（2026-08-27，v1.0.19 双平台发布实测）：api.github.com 可达、github.com:443 被墙时用 Git Data API 逐提交重放（blob→tree→commit→update ref），但**远端重建的提交 SHA 与本地不同**；若上次推送只推到一半（如只重放了 `fc8552f`，远端 master=改写版 `13f46190`），本地 `git rev-parse origin/master` 仍是旧值 `fee50ef`（API 推送不更新本地 tracking ref），而远端 SHA 不在本地对象库 → `rev-list 13f46190..master` 报 `Invalid revision range`。**正解（`runtime/tmp/git_push_github_api.py` 已实现）**：①先用 `find_local_ancestor` 沿远端提交链（API GET parents）**向上找到第一个本地对象库存在的提交**作 rev-list 基准——注意**不能只取远端 master 的 `parents[0]`**，多级改写链下父提交也可能是改写版（本地没有，实测 v1.0.19 二次续推：master=de53a965 的父 45e2cba 不在本地）；②在候选里用 `find_base_local` 按 **tree 相等**（`get_commit(remote)["tree"]["sha"]` vs `rev-parse c^{tree}`）找出远端 master 对应的本地提交，建立 `本地SHA→远端SHA` 映射；③**待推列表用 `rev-list --reverse local_base..master` 精确取 local_base 之后**（候选列表可能因本地对象库混入远端改写提交而含已推送旧提交，如曾混入 9e7ce468/13f46190 导致 rev-list 基准不准）；④`create_commit` 的 parents 用**远端父 SHA**、每次 commit 后回读远端 tree 作下一提交的 base_tree。注意：这些改写提交**只存在于远端**，本地 update-ref 到该 SHA 会报 `nonexistent object`（别尝试），本地 `git status` 显示 ahead 属预期。Gitee 侧另一教训：**Gitee 的 git data 只支持写（POST blob/tree/commit、PATCH ref），GET `/git/commits/{sha}` 返回 405**；且 gitee.com 直连可达 → Gitee 无需 API 续推，直接 `git push --force gitee master` 把完整本地历史同步过去即可（改写链无 tag 引用、是残缺中间态，force 覆盖安全；先 `git ls-remote gitee refs/heads/master` 核对远端状态再动手）。
42. **`sctx.settings.mutate` 只认 `set` / `unset`，不认 `remove`**（2026-08-27，dsh-ollama「一键接入」首次点按即报错实测）：调用 `sctx.settings.mutate("<namespace>", [{op:"remove",...}])` 时，底层 schema 校验直接抛 `settings mutate for "<ns>" ops must be (op:'set'|'unset', path)` — 异常冒泡成 `runDetection` 失败 → 客户端报"一键接入失败"。官方文档 `@deepseek-ai/dsh-host-settings` 的 `SettingsMutateOp` 定义为 `{op:"set"|"unset", path: string[], value?: any}`，**没有 `remove`**。坑 34 里写的「API 鉴权头 set / 坑 36 里 apiKeyEnv remove」等自愈逻辑必须统一改 `unset`。凡是做 settings 写入的插件，所有"删除字段"的写法都应用 `unset`。**验证方式**：看到这条错误直接搜 `op: "remove"` 改成 `unset` 即可定位。
...
- dsh-ollama：插件自身配置（baseUrl / displayName / 探测间隔等）支持两途径：`cordis.patch.yml` 覆盖 + WebUI「设置 → Ollama 设置」面板在线修改（2026-08-27 已完成，见第二节）。**「一键接入」按钮（2026-08-27）**：本机更新绿色版后曾出现"自动探测没把 Ollama 模型配置好"的情况，面板自动识别是后台周期探测、缺主动入口 → 新增独立路由 `POST /__dsh/ollama/reconnect`（与 `/config` 不同 path，规避坑 34），复用 `runDetection(force:true)` + `configPayload()`，客户端状态卡新增「一键接入」按钮触发，不保存表单、不改动 `ollama-config.json`。已同步源码 + 运行副本（MD5 校验一致）+ 便携 node `--check` 通过。**剩余待办**：重启 dsh 服务后在 WebUI 目验按钮可用（按钮在状态卡「已接入/未接入」徽章右侧），实机点击验证在线/离线两条分支。
- **社区插件生态情报与推荐口径（2026-08-25 更新 README）**：DSH 生态已爆发（精选 293 款 / 总星标约 5.2 万，总量 1700~3000 款），但**笔记本上只记录目录站与口径，不逐版本记 star**；README 内置插件表（8 款）之后已有「社区精选插件」小节。**权威目录站**：`dsh-plugins.top`（快照 2026-08-28）、`deepseek-harness-plugin.com` / `dshplugin.online`（中英、可按星标/最新排序）、`awesome-deepseek-harness`（GitHub 榜单）。**口径纪律**：星标取多站交叉、标注"约 + 快照时间"，不写精确到个位；区分"内置插件"与"社区外部插件"；必须保留"第三方插件即以你的身份在本机运行，装前看源码、别在含密钥目录乱试"的安全提醒。安装：绿色版「插件管理」页可装，或 `dsh plugin --profile web add <name>` / `<owner/repo>`，装完重启服务。**发版提醒（坑 16 同理）**：外部插件改动不涉及运行副本同步，但新增推荐如涉及内置插件安装，需走插件管理即可。