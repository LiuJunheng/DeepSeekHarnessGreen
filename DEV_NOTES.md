...
54. **【局域网手机连不上 / 防火墙按程序放行漏掉 node.exe】（launcher.py 自动放行端口，2026-08-18）**：
    - **现象**：局域网模式（`dsh_host=0.0.0.0`）下，电脑本机浏览器正常，但**手机用内网 IP 打不开**；`netstat` 显示服务已 `0.0.0.0:3080 LISTENING`，`ipconfig` 本机局域网 IP 正常，网络为 DomainAuthenticated 且防火墙规则无分域遗漏。
    - **根因**：既有防火墙规则 `dsh_launcher` 是**按「程序(dsh_launcher.exe)」放行**（`netsh ... program=E:\deepseekharnesslauncher\dsh_launcher.exe`），但实际监听 3080 的是 **`node.exe`**（官方 dsh 是 node 进程）。Windows 按程序放行只对该 exe 生效，`node.exe` 入站没有放行记录 → 局域网流量被防火墙丢弃；本机能连是因为本机回环流量不走防火墙入站过滤。
    - **修复（launcher.py）**：新增 `ensure_firewall_port(port)`，改为**按端口放行**（`netsh advfirewall firewall add rule name="DSH Green 3080 <port>" dir=in action=allow protocol=TCP localport=<port>`），并对同规则名**先 delete 再 add** 保证幂等落到最终态（同名多条 netsh 会累加重复）。在 `start_server()` 里仅在 `dsh_host==0.0.0.0` 时调用（本机模式不开放）；非 Windows / 无管理员权限 / 命令失败时仅记警告不阻断主流程。绿色版思路：不依赖用户手动设置，每次启动服务自动放行，任意电脑都能用。
    - **验证**：`py_compile`（项目便携版 Python 3）通过；生产同命令手动执行 `exit=0`、规则成功写入；`Test-NetConnection 192.168.32.88 -Port 3080` 返回 `TcpTestSucceeded: True`。局限：本机连本机局域网 IP 多走回环协议栈，未严格走防火墙入站过滤，**最终需手机实测**确认。
55. **【会话导入插件】dsh-session-import：把官方导出的会话 ZIP / .jsonl 导入回本机（2026-08-19）**：
    - **背景**：用户问「导出后没有导入功能，能否加」——官方只有导出（`/export` 按钮 → `GET /api/session.export` → ZIP），没有任何导入/恢复接口（`resumeSessionId` 只能恢复本机已持久化的会话，不是导入外部 JSONL）。按既有内置插件模式（bundle patch + 双端）新增 `plugins/dsh-session-import/`，与官方导出互逆。
    - **实现（宿主端 `lib/index.js`）**：路由 `GET /__dsh/session-import/health` + `POST /__dsh/session-import/upload`（均要求 `x-dsh-session-import: 1` 自定义头防跨站伪造；体积上限 512MB）。ZIP 按 `PK` 魔数自动识别（fflate `unzipSync`），其余按明文 JSONL 处理；校验每行 JSON 与首行 header（`type:"session"`、版本 == `SESSION_FORMAT_VERSION`、无已退役 `sandboxMode`/`approvalPolicy` 字段）；写盘布局**逐字节复刻官方持久化后端**（`dsh-session-persistence-jsonl`）：目录 `sessions/<projectKey(cwd)>/<encodeSegment(id)>/session.jsonl.zstd`（校验和 zstd 帧：header 帧 + 事件帧，`zstdCompressSync(text, {params:{[ZSTD_c_checksumFlag]:1}})`；压缩方式按根编码探测——根里有明文 `.jsonl` 且无 `.zstd` 时为 none，否则 zstd）；`subagents/*/session.jsonl` 逐份导入；`media/<attachmentId>.<ext>` 校验 sha256 后按内容寻址写回 `attachments/v1/objects/<前两位>/<sha256>`；随后按 `header.cwd` 匹配/创建工作区并 `attachSession`（cwd 目录在本机不存在则跳过挂载，会话留「未分组」但仍可见）。幂等：已存在会话 id 直接跳过不覆盖；坏文件 / 损坏 ZIP / 版本不支持一律 400 拒绝。
...
| 内置插件 dsh-session-import | `plugins/dsh-session-import/`（v0.1.0）：宿主端 `lib/index.js` 注册 `GET /__dsh/session-import/health` + `POST /__dsh/session-import/upload`（均要求 `x-dsh-session-import: 1` 防跨站；512MB 上限）：ZIP（PK 魔数，fflate unzipSync）或明文 JSONL → 校验首行 header（type/version/id/无退役 policy 字段、每行 JSON）→ 逐份写回 `sessions/<projectKey(cwd)>/<encodeSegment(id)>/session.jsonl(.zstd)`（zstd 帧与官方持久化后端逐字节一致，压缩按根编码探测）→ `media/*` 按 sha256 内容寻址写回 `attachments/v1/objects/<前两位>/<sha256>` → 按 header.cwd 匹配/创建工作区并 attachSession（目录不存在则留未分组）；重复 id 跳过不覆盖。客户端 `lib/client.js` 注入 `settings.section`「会话导入」页（文件直传 + 结果展示）。与官方导出互逆；导入即出现在会话列表（session.list 合并冷会话），无需重启。安装：`--install-plugin plugins\dsh-session-import`，重启生效 |
...
97. **【修复】启动"双界面"根因=官方 dsh 默认自动打开浏览器 (2026-08-20, 用户实测"选桌面=桌面+网页、选网页=2网页")**：
    ...
    - **排查**：启动器自动打开只有一条入口（`on_start→start_server→wait_and_open→open_ui`），`desktop-shell.py` 单开一窗，config 也正常（open_method=desktop）。顺着"固定有一个网页"这条线追到**官方 dsh-web-app 自身**：
      - `dsh-web-app/lib/startup.js`: `option("--no-open", ...)`、`openBrowser: options.open`（默认为 true）。
      - `dsh-web-app/lib/index.js`: `openBrowser: z.boolean().default(true)`；第 174 行 `handoffBrowser = config.openBrowser && ...`，第 201 行 `handoffBrowser` 为真时 `internals.openBrowser(webUrl)` 自动拉起系统默认浏览器。
...
104. **【内置插件】背景影画 dsh-media-background：本地目录视频做 WebUI 背景（2026-08-24，复刻自第三方 huawaimen/Olivia-Lin-in-DeepSeek-Harness 的核心播放能力）**：
    - **需求**：选一个本地目录 → 自动列出里面视频 → 逐条加入播放清单 → 全屏背景层播放**画面 + 声音**（音量/上一首/下一首/暂停/停止/背景浓度/循环）。目录选择入口在网页「🎬 观星」面板的目录浏览弹窗里（用户明确：不在启动器 GUI 加选择）。
    - **架构（聚焦，砍重）**：单插件自包含，**不复刻** Olivia 的三层解耦（独立 media-server 独立端口 / 待机 / 时段 / 遮挡门帘），复用本绿色版既有 `plugins/<name>/` 内置插件机制（launcher `bundled_plugin_dirs()`/`install_bundled_plugins()` 自动扫描安装，无需改这两处）。
    - **宿主端 lib/index.js**：`inject=["webServer"]`，经 `ctx.webServer.register` 注册同源路由（复用 dsh-file-browser 正确写法 `ctx.effect(() => register(...), 说明)`）：
      - 目录来源优先级：① `DSH_HOME/media-background-dir.json`（面板里 POST 改过，持久）→ ② `process.env.DSH_MEDIA_BG_DIR`（可选，环境变量默认）→ ③ 空（前端提示未配置）。**目录主要由观星面板设置并持久化**，launcher 不再注入（GUI 不提供选择入口）。
      - 路由：`GET /config`、`POST /config`（校验 `stat` 是目录后写持久化）、`GET /list`（**递归扫描**含子文件夹，深度最多 `MAX_SCAN_DEPTH=6`，扩展名过滤 `mp4/webm/m4v/mov/ogg`，子目录视频以 `子目录/文件名` 命名，返回 name/size/url）、`GET /browse?path=`（目录浏览：path 缺省=列盘符 Windows；带 path=返回该目录直接子文件夹名，供前端逐层下钻）——这四个**带守卫头 `x-dsh-media-bg`**（无头 403，防外部网页触发本地目录读写/浏览）。
      - `GET /stream?path=<rel>`：视频 Range 流——`decodeURIComponent` → `path.resolve(dir, rel)` → **`isInside(dir)` 防穿越**(否则 403)；解析 `Range`（`bytes=start-end` / `bytes=-N`) 用 `createReadStream(file,{start,end})` 返回 `206`+`content-range`/`content-length`/`content-type`，无 Range 返回 `200`+完整长+`accept-ranges: bytes`；**该路由不要求守卫头**（浏览器 `<video>` 无法带自定义头，加了就播不出来），改由"目录内防穿越 + 仅限已配置目录"兜底。
    - **客户端 lib/client.js**：`window.__ModuleLoader__.load` 直接往 `document.body` 挂命名空间节点（不依赖官方内部布局插槽）。全屏 `<video>` 背景层 `position:fixed; inset:0; z-index:0; object-fit:cover; pointer-events:none`，默认隐藏；点右下角浮钮「🎬 观星」开/收面板；`opacity` 滑块（0~100）控通透度＝默认 35% 半透明壁纸，对话内容透出；音量/上一首/下一首/暂停/停止/循环、视频列表行点击试播/＋加入清单/清空清单、清单可删单条。**目录选择**：面板目录行加「选目录…」→ 弹出覆盖层 `#dsw-mbg-browse`（从"我的电脑"盘符开始，逐层下钻/返回上层/「使用此目录」确认，调 `/browse` 拿子目录），也可直接粘贴路径。**清单默认循环**：加入第一首后自动开始播放；播放时 `video.loop=false` 禁用浏览器原生单曲循环（否则 loop=true 时 `ended` 不触发、手动清单循环永远不跑），由 `onEnded` 负责清单循环（末尾回 0）。清单/音量/浓度/循环存 `localStorage`（key `dsw-mbg-v1`）。
    - **launcher.py 对接（绿色版轨道，2026-08-24 撤销）**：第一版在 `DEFAULT_CONFIG` 加了 `"media_background_dir"` + GUI「常规设置」加"背景视频目录"行（`filedialog.askdirectory`）+ `build_env()` 注入 `DSH_MEDIA_BG_DIR`；**用户反馈不要 GUI 加目录，改在观星面板弹窗选** → 撤销这 4 处（config 字段 / `build_env` 注入 / GUI 变量与按钮 / `sync_gui` 写入行），launcher.py 恢复无目录相关。目录完全由观星面板设置并持久化到 `media-background-dir.json`。
    - **补充：原生选目录 vs 浓度边界 vs 视频层级（同 2026-08-24）**：受用户反馈演进——①**「选目录…」优先弹 Windows 原生文件夹选择框**：新增 `GET /pickdir?path=<当前目录>` 路由（守护头保护），宿主端 `pickNativeDir()` 用 `execFile` 跑临时 `pickdir.ps1`（.NET `FolderBrowserDialog`），**初始目录经环境变量 `DSH_PICK_INITIAL` 传入绕过脚本内联字符串的转义，ps1 为纯 ASCII 避免 PowerShell 编码乱码，powershell 用 `SystemRoot` 拼完整路径**，选中返回路径、取消/非 Windows/失败返回 `{fallback:true}`；客户端 `openBrowse` 改为 async：先 `fetch /pickdir` 成功即回填应用（不再开网页弹窗），失败才回退原逐层浏览弹窗 `#dsw-mbg-browse`。②**「浓度」只改变视频那层 opacity**：滑块仅调 `S.opacity → applyVideoAppearance()`（只设 `videoEl.style.opacity`）。③**避坑：视频层级压字（用户反馈"覆盖部分工作文字"）**——视频曾 `appendChild` 到 `<body>` **尾部**，与 harness 工作区同为 `z-index:auto/0` 普通层时**后者绘制在上**，视频反盖住了工作区文字，浓度拉高更明显；**修法**：改 `document.body.insertBefore(videoEl, document.body.firstChild)` 插到 `<body>` **最前**，让工作区文字永久浮于视频之上（浓度只控视频透明度、不遮字）。④**避坑：整窗背景板盖住视频（用户反馈"视频完全被挡住、背景板盖在视频上方"）**——仅把视频插到 `<body>` 最前仍不够：WebUI 挂载点是 `<div id="root">`，其**外壳容器用 `var(--dsw-alias-bg-base)` 画了一张不透明的整窗背景板**，DOM 顺序在视频之后、同为普通层时把 `z-index:0` 的固定视频整个盖住。**修法（动态透明化）**：新增 `setBackgroundActive(on)` 给 `<html>` 加/去 `dsw-mbg-active` 类；CSS `.dsw-mbg-active` 下用 `!important` 把 `body`、`#root` 及**页面底座变量 `--dsw-alias-bg-base`** 全部置为透明（这些变量在 `body[data-ds-dark-theme]` 上定义、随 CSS 变量继承到外壳）。**后续优化（用户实测反馈"左下设置界面也被透掉、影响操作"）→ 刨除 layer 系列**：设置面板/悬浮层用的是 `--dsw-alias-bg-layer-2`（`dsh-client-ui-settings-general` 的 `SettingsRoot.module.css` 面板背景 `background:var(--dsw-alias-bg-layer-2)`；浮层弹层用 `--dsw-specific-menu`、遮罩用 `--dsw-alias-bg-mask-*`），这些**一律不再强制透明**，故设置界面等始终不透明可操作；而聊天画布/空白壁纸用 `--dsw-alias-bg-base`（会话插件内只有 `background:var(--dsw-alias-bg-base)`，无 layer-1 用作大容器背景），故只透明 base 视频仍能整窗透出。视频开始播放（`playListIndex`/`playPreview`）时置真、停止（`stop`）时置假。**效果即用户预期**：未播放用回 harness 原生纯色壁纸兜底；播放时页面背景全透明只剩视频、设置/弹层/侧栏保持不透明、对话文字恒在上。
    - **安全边界**：目录受限读（只读用户配置的那一个目录）；`stream` 对 `..` 越界一律 403；`/config`、`/list`、`/browse`、`/pickdir` 带守卫头才放行。背景透明化**只在播放视频时动态进行**（`<html>.dsw-mbg-active` 临时覆盖 `--dsw-alias-bg-base` 等为 transparent），一停止立即移除类复原——比 Olivia 原版"常驻整体透明化透出壁纸"更克制，空会话仍保持原生深色纯色壁纸兜底。
    - **文案包装（三国）**：插件/面板名「观星」——呼应军师于帷幄之中观星察势、选群雄（视频）轮流当值作为帐帘映衬。
    - **避坑：原生选目录弹窗被其他窗口遮挡（2026-08-24 补）**：`FolderBrowserDialog.ShowDialog()` **无 owner 时对话框以无主顶层窗口弹出，Windows 不会强制置顶**，常被其他窗口盖住、用户以为没弹出。**修法**：`pickdir.ps1` 里 `Add-Type` 内联 C# 声明 `user32.dll` 的 `GetForegroundWindow()`，取当前前台窗口句柄后用 `System.Windows.Forms.NativeWindow` 包装成 `IWin32Window` owner 传给 `ShowDialog($nativeWindow)`（用后 `ReleaseHandle()`），对话框即模态于前台窗口之上置顶弹出；无前台窗口时回退无 owner 的 `ShowDialog()`。脚本仍为纯 ASCII。经验：**凡 Node 进程用 execFile 弹 .NET 对话框，都要给 ShowDialog 传前台窗口 owner，否则必被盖住**。
    - **文档同步**：新建 `plugins/dsh-media-background/README.md`；根 `README.md`/`README_EN.md` 内置插件从 6 款升 **7 款**（一览表+详解新增"背景影画"分组+协议/目录结构/亮点共 4 处数字）；`DEV_NOTES.md` 本条目；`skills/dsh-deploy-maintain/` 的 SKILL.md/README 同步内置插件清单与本插件经验。
    - **未做（可后续）**：mkv/avi/HEVC 等浏览器不原生支持的编码需转 mp4/webm；不进三级以上深层大目录（扫描深度限 6 层防性能/循环）；不做待机轮播/时段/遮罩门帘等高级特性。

105. **【内置插件增强】观星背景影画：更多视频格式 + mp3/wav 纯音乐模式（2026-08-24，需求 #104 后续）**：
    - **需求**：用户体验后发现仅支持 mp4 不够——① 希望能播更多视频格式；② 希望支持 mp3/wav 等纯音频"**纯音乐模式**"（只出伴奏、不挂黑屏壁纸）。
    - **宿主端 lib/index.js**：把扩展名集拆成两套——`VIDEO_EXTS`（原 mp4/webm/m4v/mov/ogg，**新增 mkv/avi/flv/wmv/mpg/mpeg/ts/3gp/m2ts**）与 `AUDIO_EXTS`（**mp3/wav/flac/aac/m4a/wma/opus/amr**）；`MIME` 表补全两套对应的 `content-type`（含 `audio/mpeg`、`audio/wav`、`video/x-matroska` 等），`stream` 端直接查表返回；`listVideos` 扫描过滤条件放宽为「视频集 ∪ 音频集」，并给每个文件加 `kind` 字段（`AUDIO_EXTS.has(ext) ? "audio" : "video"`），前端据此区分呈现方式。
    - **客户端 lib/client.js**：媒体项结构增加 `kind`——扫描列表、加入清单、试播项都带上；`loadStore` 读旧版无 kind 的 localStorage 清单时用 `Object.assign({kind:"video"}, item)` 兼容。新增 `currentItem()`（试播优先、其次清单当前项）与 `applyMediaMode()`：`isAudioMode = (item.kind==="audio")` 时 `setBackgroundActive(false)`（恢复原生背景板不透明）+ `videoEl.style.display="none"`（只出声、不显示画面）；否则视频模式 `setBackgroundActive(true)` + `display` 复位 + 恢复 `opacity`。`playListIndex`/`playPreview`/`stop` 三处统一改由 `applyMediaMode()` 驱动（替代原来硬编码 `setBackgroundActive(true)`）；`stop()` 里补 `videoEl.style.display=""` 复位，避免下次视频被"纯音乐隐藏"残留影响。
    - **避坑（纯音乐黑屏）**：音频若继续走"背景板全透明 + `video` 显示"路径，`<video>` 背景是 `#000`，整窗就变成黑底听歌——所以纯音乐必须恢复原生背景板并隐藏 `<video>` 画面，保留 DeepSeek Harness 原深色壁纸作视觉。
    - **避坑（解码不了的格式）**：mkv/flv/wmv 等能否解取决于浏览器内核与编码，统一列出并交给浏览器尝试；无法解码会触发异常/结束事件，前端 `onEnded` 自动切下一首，不影响清单其余项。故不再硬性"先转 mp4"，用户可直接扫一整个素材目录。
    - **文档同步**：`plugins/dsh-media-background/README.md`「目录识别」改「目录识别与纯音乐」（列全两套扩展名、纯音乐说明）；`DEV_NOTES.md` 本条目。语法校验：便携 node `--check` 两文件全过。

106. **【内置插件全量】UI 跟随 DSH 深/浅主题自适应（2026-08-24，需求 #106）**：
    - **现象**：7 个内置插件 WebUI 面板/表格/按钮/弹窗几乎全硬编码颜色（约 226 处、且 0 处主题判断），切深色时黑字落深底看不清。
    - **机制（关键）**：harness 在根容器按 `body` 与 `body[data-ds-dark-theme]` 各定义一套 `--dsw-alias-*` 语义变量（见 `runtime/dsh/node_modules/@deepseek-ai/dsh-client-ui-theme/lib/client.js` 第 124 行 design-platform.css）。**插件把硬编码色换成 `var(--dsw-alias-*)` 后，CSS 变量级联自动随深浅主题切换取值，无需任何 JS 监听/主题判断**。
    - **变量族（色→语义）**：主文字 `label-primary`、次文字 `label-secondary`、弱文字 `label-tertiary`；面板/浮层背景 `bg-layer-2`（更浅 layer-1、更深强调 layer-3）、底座 `bg-base`；边框 `border-l1/l2`；行 hover/选中 `interactive-bg-hover(-solid/-active)`；主按钮 `button-primary-fill`、品牌 `brand-primary`；语义色 `state-success/-error/-warn(-primary/-secondary/-label)`；遮罩 `bg-mask-1/2/3`；滚动条 `--dsh-scrollbar-thumb` 或 `--dsw-alias-scrollbar-bg-l1`。**避免直接引 `--dsw-static-*`（静态色不随主题）**做底色/文字/边框。
    - **改造清单**：`dsh-archive-purge`(≈13)/`dsh-session-import`(7)/`dsh-session-rewind`(≈31)/`dsh-usage-stats`(底色/文字全换)/`dsh-sidebar-lite`(40+)/`dsh-media-background`(仅面板控件)/`dsh-file-browser`(**本就全量 `var(--, 回退值)` 编写，无需改**)。语法校验 7 个 client.js 全过。
    - **四避坑**：
      1. **media-background 免碰背景透明语义**：`html.dsw-mbg-active`、`setBackgroundActive`、`applyMediaMode`、`--dsw-alias-bg-base:transparent`、`<video> background:#000` 一路原样保留；只把「观星」面板控件色变量化。
      2. **状态装饰可留少量静态色**：运行蓝 `#1a56db`、成功绿 `#16a34a`、危险红、品牌强调 `#4a7bff`（激活 tab、主按钮白字）与阴影——它们在深浅主题下对比度仍达标（如 usage-stats 状态徽标浅底+深字），不必强改。
      3. **不要加 JS 主题监听**：纯 CSS 变量级联即自动跟随，加了反而还与主题初始化时序冲突。
      4. **别漏「缺显式 color、靠继承」的文字**（2026-08-24 补漏）：纯样式替换后仍有零星文字在深色下发黑，多是无 `color`、靠父级继承的正文/标题/数值/折叠标题（如 `dsh-session-import` 区块描述与导入结果、`dsh-usage-stats` 的标题/数值卡/「价格表」summary）。这类节点在深色下继承链会落到未主题化默认黑，**需显式补 `var(--dsw-alias-label-*)`**，别只依赖 harness 面板根的继承色。

107. **【避坑 #74 补】主题改造改了 `plugins/` 源码但运行副本没同步 → 深色下白框依旧（2026-08-25 实测）**：
    - **现象**：#106 主题化改造后，用户切深色主题仍见用量统计里大部分文字被**固定白底框**框住（会话卡 `#fff`、chips `#f5f5f5`、回合块 `#fafafa`、总览/价格卡 `#fff` + 边框 `#ddd`），看起来完全没随主题变。
    - **根因**：内置插件经 `dsh plugin add file:`（pnpm）**拷贝**进 `runtime/dsh-home/profiles/web/node_modules/<插件>/`（见避坑 #30），dsh 实际运行的 bundle 从**安装副本**读取；#106 只改了 `plugins/` 源码，从未同步到安装副本 → **整个主题改造在运行端根本没生效**。实测 7 个插件里仅 3 个运行副本过期（`dsh-archive-purge`/`dsh-session-rewind`/`dsh-usage-stats`），其余 4 个（session-import/sidebar-lite/media-background/file-browser）恰好已是同步版本。
    - **修复（同步运行副本）**：把 `plugins/<name>/` 全部内容镜像覆盖到 `runtime/dsh-home/profiles/web/node_modules/<name>/`。**坑 A（PowerShell 嵌套）**：`Copy-Item -Path $src -Destination $dst -Recurse` 在 `$dst` 已存在时会把 `$src` 目录**整个嵌进 `$dst` 里**（生成 `<dst>/<name>/…`），必须改用 `Copy-Item -Path "$src\*" -Destination $dst -Recurse -Force` 复制内容；**坑 B（文件锁）**：dsh 服务运行中 `lib/index.js`、`cordis.patch.yml` 被进程占用无法覆盖（客户端 `lib/client.js` 按请求生成、不受锁）。同步后 `Get-FileHash` 逐文件比对全部 SAME。
    - **生效方式**：纯客户端改动（client.js）**浏览器强刷**即生效；服务端文件（index.js/cordis.patch.yml/package.json 有变时）需**重启服务**。本次 session-rewind 连 `index.js`/`cordis.patch.yml`/新增 `tools/` 一并同步，建议重启服务。
    - **教训（重要）**：**改 `plugins/` 源码后必须同步运行副本（或重装插件），否则改动静默不生效**——此前所有"主题化已完成"的判断都只验了源码、没在运行端目验。后续开发流程应把「同步 `runtime/dsh-home/profiles/web/node_modules/<插件>/` + 强刷/重启」作为必做步骤。

108. **【内置插件】用量统计：背景框固定浅色 + 框内文字固定深色，仅页面级文字随主题（2026-08-25，需求 #106 的细化修正）**：
    - **背景**：#106 曾把用量统计整页底色/文字全换成语义变量随主题；但用量统计大量信息都放在**固定浅色背景框**里（会话卡 `#fff`、元信息 chips `#f5f5f5`、回合明细块 `#fafafa`、总览/价格卡 `#fff` + 边框 `#ddd`、表头 `#e6e6e6`）。用户明确：这些背景框颜色不变（刻意保持浅色），**框内文字若随深色主题变白 → 白字落白底看不清**。
    - **需求（用户明确）**：① 背景框**固定浅色**（不随主题变）；② 框内文字**固定深色**（不随主题变白）；③ 只在框外的**页面级文字**保留主题自适应。
    - **实现**：把 #106 时替换为 `var(--dsw-alias-bg-*)` 的框背景改回硬编码浅色（`#ffffff`/`#f5f5f5`/`#fafafa`、边框 `#dddddd`/`#e6e6e6`）；把框内文字固定为深色（主文字 `#1f1f1f`、次文字 `#8a8f98`/`#555555`）；页面级文字（client.js 763 行等）仍保留 `var(--dsw-alias-label-secondary, #8a8f98)` 随主题。同步运行副本 `runtime/dsh-home/profiles/web/node_modules/dsh-usage-stats/lib/client.js`（避坑 #107 教训，`Get-FileHash` 比对 SAME）。纯客户端改动，浏览器强刷即生效。
...
110. **【发布】v1.0.16：用量统计接入 DeepSeek 真实余额 + 兼容非 DeepSeek 账户不报错落地（2026-08-25，需求 #109 收口 + 双平台 Release）**：
    - **版本**：`GREEN_VERSION` 升 1.0.16（launcher.py 与 runtime/tmp/build_release_zip.py 同步）；重打 DSH_Launcher.exe / DSH_Update.exe（build_exe.bat 3b/3c，固化当天日期）；跑 build_release_zip.py 生成 `DSH_Launcher_GreenPortable_Online_20260825_v1.0.16.zip`（~17MB，含 plugins/、skills/dsh-deploy-maintain/，不含 runtime/、DEV_NOTES、.gitignore）。
    - **源码/文档推送**：`git commit`（d8a8b63）+ `git tag v1.0.16`，用 `git -c http.proxy=http://127.0.0.1:10809 -c https.proxy=http://127.0.0.1:10809 -c http.postBuffer=52428800 -c http.lowSpeedLimit=0 -c http.lowSpeedTime=999 push` 推送 master 与 tag（本机全局 git 代理 `127.0.0.1:10809`，去 lowSpeedLimit 避免低速超时，记忆避坑 #96）。
...
112. **【发布】v1.0.17：绿色版更新检查 Gitee 排序修复落地（2026-08-26，需求 #111 的修复 + 双平台 Release）**：
    ...
    - **git**：commit（launcher.py 版本号 + DEV_NOTES + 两个 exe `DSH_Launcher.exe`/`DSH_Update.exe` 一并跟踪）+ `git tag v1.0.17`。**注意**：本机到 `github.com:443` 的 git 直连超时不可达，而 `api.github.com` 可访问——改用 GitHub API `POST /repos/{owner}/{repo}/git/refs` 创建 `refs/tags/v1.0.17`（指向提交 68967e4）完成 tag 推送；master 仍是 git 正常 push。故"tag 推不上 github.com"时，可用 API 建 ref 替代。
    - **双平台 Release 落地结果**：GitHub Release `v1.0.17`（id=376972026）+ 上传 zip 资产（id=530495275，17,703,580B）；Gitee Release `v1.0.17`（id=901649）+ 上传 zip（id=3098611，HTTP 201）。Gitee 创建 Release 用 `POST /releases` + `Content-Type: application/x-www-form-urlencoded`，**必须带 `target_commitish`**，否则 400；上传附件用 `curl -F "file=@..."`（本机 PowerShell 版本不支持 `Invoke-RestMethod -Form`）。
    - **Gitee 旧 zip 清理（用户明确要求，已全部完成）**：Gitee 历史 v1.0.9~v1.0.16 共 8 个 Release 上的**所有手动上传附件（9 个：v1.0.9 两个含 skill zip、v1.0.10~v1.0.16 各一个绿色 zip）全部删除，只剩最新 v1.0.17 的 zip**。删除走 Gitee API `DELETE /repos/{owner}/{repo}/releases/{id}/attach_files/{asset_id}`（需 access_token）；保留的 tarball/zipball（GitHub/Gitee 自动生成、`archive/refs/tags/...`）属源码包，策略上不删。删除后 `attach_files?per_page=100` 逐条复查：v1.0.9~v1.0.16 均为空，仅 v1.0.17 保留 zip。
    - **经验（Gitee 删附件的幂等/限流坑）**：`DELETE attach_files` 用 **PowerShell `Invoke-RestMethod -Method Delete` 会 404**，改用 **curl.exe** 正常返回 204。且**短时间循环批量删除会命中限流返回 404（实际可能成功也可能未删），单独逐条删除稳定 204**——务必逐条删并 `list_releases`/`attach_files` 复查，别信批量循环的 404 即"失败"。另注意：`GET /releases` 返回**升序**且每页默认 20，定位最新/遍历历史都要 `per_page=100` 后再自行处理顺序。
