...
110. **【发布】v1.0.16：用量统计接入 DeepSeek 真实余额 + 兼容非 DeepSeek 账户不报错落地（2026-08-25，需求 #109 收口 + 双平台 Release）**：
    ...
    - **Gitee Release（已建，资产待补）**：用 Gitee MCP `create_release`（owner=`liujunheng`、repo=`DeepSeekHarnessGreen`、tag=v1.0.16、target_commitish=master）创建成功（id 892624），但 Gitee MCP **只有 create_release 没有上传资产工具**，自动生成的只是源码 `refs/tags/v1.0.16.zip/.tar.gz`，**没有绿色版 zip**。因绿色版国内源/自动优先走 Gitee Release、`green_find_zip_asset` 按前缀 `DSH_Launcher_GreenPortable_Online_` 匹配 zip——**Gitee 缺该 zip 时国内源会匹配失败，回退到 Gitee 整仓 git 克隆快照**。**待办：作者需到 gitee.com 该 Release 页面手动上传 `DSH_Launcher_GreenPortable_Online_20260825_v1.0.16.zip`**（或日后给 Gitee 侧配置能上传资产的 token）。经验：GitHub / Gitee 双发布，release 建法与资产上传能力不对称——GitHub 有 `uploads/.../assets` API + GH_TOKEN 可自动传，Gitee 需第三方 token 走 `attach_files` 上传，工具侧暂缺，只能手动。
