...
- **版本**：`GREEN_VERSION = 1.0.20`（launcher.py 内，唯一版本来源；`GREEN_VERSION_DATE` 打包当天自动回写；`build_release_zip.py` 已改为**从 launcher.py 读取版本号**，不再硬编码脚本内版本，避免 zip 名与真实版本不一致）。GitHub + Gitee 双平台已发到 v1.0.19（2026-08-27：GitHub Release id=377753678 / Gitee Release id=916774，zip 17,745,739 B）；v1.0.20（2026-08-27，新增 dsh-ollama「一键接入」按钮 + 修复 `sctx.settings.mutate` op 类型，见坑 42）。
