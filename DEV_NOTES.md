...
* **版本唯一来源 = launcher.py** **`GREEN_VERSION`**；`GREEN_VERSION_DATE` 由 `release_upload.py` 打包当天自动回写（禁止预写未来日期）。zip 名与发布均以此为准。当前已发布 **v1.0.39**。
...
7. **对话框开着时最小化主窗口 → 点任务栏图标完全没反应（像卡死），点托盘却正常**（2026-09-25 用户实测 + 三组 Win32/Tk 探针定位）。这条踩过两轮：第一轮按"Windows 随 owner 隐藏子窗口"的错误假设去修（治好了"检查过程中最小化"那半），第二轮才用探针把真实机制拆开。**实测机制（Tk 8.6 + Windows，三条彼此独立）**：① `root.iconify()` 后，Tk **自己**会把 `transient(root)` 的子窗口状态改成 `withdrawn`（不是 Windows 干的），而 master 恢复时 Tk **不会**把子窗口一起还原 —— 对话框永久停在 withdrawn；② **只要子对话框握着 `grab_set()`，Tk 就会丢弃 Windows 发来的 `SC_RESTORE`** —— 主窗口永远停在 `iconic`，点任务栏图标彻底没反应（**这才是用户症状的主因**；对照组：释放 grab 后同一个 `SC_RESTORE` 立刻能把主窗口和子窗口都恢复）；③ 后台任务回调（如「检查更新」跑完弹「版本选择」）在窗口已最小化时弹框，该对话框一出生就是隐藏态且已抓走输入。**修复（四层，见 launcher.py 模块级 `_ACTIVE_MODAL_DIALOGS` 区块 + `SysTrayIcon` 窗口过程）**：① 弹框前 `_restore_owner_window()` 先恢复最小化/隐藏的 master；② `_register_modal_dialog()` 用 `after_idle` 显式 `deiconify + lift + focus_force`（放 idle 里避免"先显示空窗再被控件撑大"的闪烁）；③ **在 `SysTrayIcon` 的 WndProc 里拦截 `SC_RESTORE`，但加条件 `and _ACTIVE_MODAL_DIALOGS`** —— 有 grab 对话框时置 `_restore_pending` 交给 poll，由 Tk 侧 `deiconify()` 恢复（`deiconify` 不受 grab 影响，且会把 ① 造成的 withdrawn 一并带回 normal）；无对话框时**不拦截**，Tk 原生恢复行为完全不变；④ `_reshow_active_modal_dialogs()` 把登记表里所有非 `normal` 状态的对话框拉回 normal。恢复入口两处：`restore_from_tray`（托盘左键，有对话框时**必须先抬对话框再 return**，否则会去开桌面/网页窗口跟 grab 抢焦点）与 `root.bind("<Map>", ...)`（兜住未拦截的恢复路径）。
   **原生 messagebox 同理但处理方式不同**：`tk_messageBox` 内部自己建 Toplevel、弹完即销毁，没有可登记的窗口对象，而调用点散落在十几处后台回调（安装/启动/停止/清理/更新完成）→ 用 `_RestoringMessagebox` 在 `run_gui` 里把 `messagebox` 这个**局部名字**换成包装对象，`showinfo/showerror/showwarning/askyesno` 前自动恢复主窗口，其余属性由 `__getattr__` 透传（行为完全一致，也不漏将来新增的调用点）。注意 `run_gui` 里单实例检测分支那次 `showwarning` 发生在 `root` 创建之前、且显式传了 `parent=root_tmp`，不受替换影响。
   **方法论教训（比结论更值钱）**：① 不要凭"Windows 应该会怎样"猜窗口行为 —— 这次第一轮就是猜错方向白修一半；写个 30 行探针用 `IsIconic` / `IsWindowVisible` / `winfo_viewable` / `grab_current()` + `PostMessageW(WM_SYSCOMMAND, SC_RESTORE)` 直接问系统，一轮就定位到真因。② **探针里模拟系统消息必须用 `PostMessageW`（异步），绝不能用 `SendMessageW`（同步）** —— 后者会同步重入本线程窗口过程，在 Python 里直接崩 `PyEval_RestoreThread: the function must be called with the GIL held`（与避坑第 5 条同源）。③ `grab_current()` 是判断"谁握着本地 grab"最快的手段；`dialog.state()` 能看出 Tk 自说自话的 withdrawn，但看不出"被系统隐藏"，两者要配合 Win32 查询一起用。④ 实测 `withdraw/unmap` **不会**释放本地 grab（`grab_current()` 全程不变）。
