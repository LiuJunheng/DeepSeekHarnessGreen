...
41. **发布约定（v1.0.19 起）：Gitee 不需要手动上传，GitHub 提交并上传即可，代码/tag/Release 会自动同步到 Gitee**（2026-08-27，用户明确）——Gitee `liujunheng/DeepSeekHarnessGreen` 已配置从 GitHub `LiuJunheng/DeepSeekHarnessGreen` 自动同步（见"仓库"条：国内镜像，代码/tag 自动同步）。因此发布流程**只需**：①推 GitHub（github.com:443 被墙时用 Git Data API 续推，见坑 40）；②建 GitHub Release + 传绿色 zip（`runtime/tmp/github_release_vNNNN.py`，422 复用已存在 Release）；③**跳过 Gitee 侧一切手动操作**（不 force-push、不跑 `gitee_release_*.py`）。核对完 GitHub 侧远端 master = 本地 HEAD、Release 附件 zip 字节数与本地一致（17745739 B）即为发布完成。Gitee 自动同步若未即时生效，属 Gitee 镜像同步延迟，无需人工干预。
