...
- **水纹动效**：`#water-background` Canvas 2D 绘制（三段流动水波带：青/蓝/紫、`lighter` 加法混合 alpha≈0.16~0.18、波幅≈8% 屏高；+ 自动与指针涟漪）；`prefers-reduced-motion` 时停用；调参集中在 `startWaterRipples` 的 `waveBands`/`maxRipples`/`autoSpawnGap`；`main,.footer` 抬升 `z-index:5` 于画布（`z-index:0`）之上保证可读。
