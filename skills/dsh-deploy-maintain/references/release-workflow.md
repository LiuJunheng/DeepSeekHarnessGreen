...
**唯一的判定标准：对比「上一个已发布版本」，使用者是否可能遇到这个问题。** 而不是"本轮开发里我改了多少东西"。

- ❌ **不写进 notes**：**本轮开发中自己改出来、又在同一轮内改回去的 bug**。例如本轮把 bat 文件编码写坏又修好、参数名写错又纠正、漏收集依赖又补上——**用户从没装过带该 bug 的版本**，写进去只会制造"这个版本又修了一堆 bug"的错觉。这是开发过程，不是版本变更。
- ✅ **要写**：修的是**上一个已发布版本里真实存在**的问题（用户装了上一版就会踩到），必须在 notes 里告知。
- ⚠️ **纯内部重构 / 死代码清理**：对外行为完全不变时，不要包装成"新增功能"或"修复"；确有必要提就用一行带过（如"内部代码清理，无行为变化"）。若清理顺带改变了**使用者可感知的行为**（例如不再往程序目录写某个日志文件），则按真实影响如实写一句。

配套纪律：

- 写 notes 前先跑 `git log v<上一版>..HEAD --oneline`，**逐条问"这条对上一版的用户可见吗"**；不可见的合并成"内部改动"或直接不写。
- 内部过程要留痕就写进开发纪要（如 DEV_NOTES.md 的避坑库 / 发版复盘），**不进 release_notes**。
- 同理，发版复盘里记录的坑（如"本轮踩了 X 又修好了"）属于过程经验，**不要回头塞进已发布的 notes**。
