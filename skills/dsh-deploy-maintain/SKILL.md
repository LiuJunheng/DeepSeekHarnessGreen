...
- **搜索结果 base：排除官方内置 + 核心兼容判定（2026-09-10 搜索入口优化）**：npm 搜索会混入大量 `@deepseek-ai/*` 官方自带插件（本机已装/内置，不该作为"可搜索的第三方"列出）——**必须在** `search_npm_plugins` 里 `if package_name.startswith("@deepseek-ai/"): continue` 过滤。**搜索入口是纯 npm 源**，可复用安装侧的"核心版本兼容"启发式：`fetch_plugin_manifest(pkg)`（走当前镜像 `/pkg/latest`，8s 超时，失败返回 None）拉 manifest，`_classify_core_compat(manifest, _host_core_versions())` 比对插件 `dependencies`/`peerDependencies` 里的 `@deepseek-ai/*` 区间与本地核心子包版本 —— 全满足=预估兼容(ok)/有区间不满足=可能不兼容(warn)/拉取失败或版本无法解析=不明(unknown)。**无核心依赖也判 ok（2026-09-10 用户语义修正）**：插件不依赖任何 `@deepseek-ai/*`，天然不受官方核心版本变化影响，兼容性更稳，等同「预估兼容」，对内置插件和第三方通用。因此 `_classify_core_compat` 对空核心依赖返回 `status="ok"`，状态只剩 ok/warn/unknown 三态（`no_core_dep` 及 `compat_nocore` 文案键随之弃用，但保留不影响）。**一个插件一个 manifest 是额外网络请求，只能给前 30 条后台实测**（`enrich_npm_plugins(max_check=30)`），超量未测的也**统一标"不明"**（不留裸版本号）。 状态列文案：兼容→**预估兼容**（别再写死"兼容"，核心是活的会过期）、一般兼容→**可能兼容**（无核心依赖，暂无核心冲突面，概率偏高）、无法验证→**不明**；i18n 键 `compat_ok/compat_warn/compat_nocore/compat_unknown` 必须 **zh.json 与 en.json 同时补**——漏 en 会在英文界面 `i18n.t` 掉回键名。搜索按钮文案「搜索npm插件」（英文 Search npm Plugins）与「加载 GitHub 热门」分开，二者语义明确。版本号解析用 `_version_tuple`（支持 `x.y.z`/`x.y.z-pre`，失败返回 None 走 unknown），区间用 `_satisfies_range` best-effort（遇 `||` 组合或无法解析返回 None，**不做否定断言**，宁可 unknown 也不误报兼容）。
  ...
  - **已安装列表也加"兼容"列（2026-09-10）**：已安装插件可直接读本地产物判定，**无需联网**。产物路径 `DSH_HOME_DIR/profiles/<profile>/node_modules/<包名>/package.json`（作用域包 `@scope/name` 是 `node_modules/@scope/name/package.json`，用 `*(package_name.split("/"))` 拼接）。新增 `classify_installed_plugin_compat(package_name, host_versions, profile)` 返回 `_classify_core_compat` 结果；`refresh_installed` 里先 `host_versions = app._host_core_versions()` 算一次再逐项传入（避免每项重复全量扫盘）。Treeview 加第 3 列 & `column_compat` i18n key（zh/en 都要加）。

  - **推荐列表也补版本+兼容（2026-09-10）**：`RECOMMENDED_PLUGINS` 里 npm 源项（`dshmarket`/`dsh-usage-stats`）也应在版本列显示真实版本号 `vX.Y.Z [预估兼容]`，与搜索结果一致；github 源项无法本地比对核心版本显示「不明」。关键坑：**推荐项的显示名 ≠ npm 包名**（`dsh-market` 的 spec 是 `dshmarket`），拉 manifest 必须用 `spec`（或 name），不能直接拿显示名去 fetch（会 404）。推荐加载从纯本地改成了后台线程（每个 npm 项一次网络请求）+ `set_plugin_busy(True/False)`。注意 npm 上发布的版本可能与本地内置不同（如内置 `dsh-usage-stats@0.2.0` 无核心依赖，npm `0.1.16` 反而声明了核心依赖），以发布为准。
